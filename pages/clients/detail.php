...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_note') {
        ...
        header("Location: detail.php?id=$client_id"); // Terug naar homepage
        exit;
    }
    ...
    <div class="bg-white border border-gray-300 shadow-sm mb-6">
        <div class="p-6 flex flex-col md:flex-row items-start justify-between">
            ...
            <?php if($user_role === 'management'): ?>
                <a href="edit.php?id=<?php echo $client_id; ?>" class="bg-white border border-gray-300 text-slate-700 hover:text-blue-700 hover:bg-slate-50 px-4 py-2 text-sm font-bold uppercase transition-colors">Wijzigen</a>
            <?php endif; ?>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white border border-gray-300 shadow-sm flex flex-col h-[600px]">
                <div class="bg-slate-800 text-white px-4 py-3 border-b border-slate-900 flex justify-between items-center">
                    ...
                    <span class="bg-slate-700 text-[10px] px-2 py-0.5 border border-slate-600"><?php echo count($chat_notes); ?> berichten</span>
                </div>
    ...
    <div id="zorgplan" class="tab-content hidden">
        <div class="bg-white border border-gray-300 shadow-sm">
            ...
            <div class="p-6">
                ...
                <?php else: ?>
                    <p class="text-slate-500 italic text-center py-8">Geen zorgtaken ingesteld voor deze cliënt.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
                    ...
                    <form action="detail.php?id=<?php echo $client_id; ?>" method="POST">
                        ...
                        <div class="mb-3">
                             ...
                             <div class="flex gap-2">
                                 <label class="border border-gray-300 p-2 flex-1 text-center cursor-pointer hover:bg-slate-50 text-xs font-bold uppercase"><input type="radio" name="mood" value="Blij" checked> Blij</label>
                                 <label class="border border-gray-300 p-2 flex-1 text-center cursor-pointer hover:bg-slate-50 text-xs font-bold uppercase"><input type="radio" name="mood" value="Rustig"> Rustig</label>
                                 <label class="border border-gray-300 p-2 flex-1 text-center cursor-pointer hover:bg-slate-50 text-xs font-bold uppercase"><input type="radio" name="mood" value="Pijn"> Pijn</label>
                             </div>
                        </div>
                        <div class="mb-3">
                        ...
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-3 text-slate-700 align-top"><?php echo nl2br(htmlspecialchars($r['content'])); ?></td>
...
<div id="taskModal" class="fixed inset-0 bg-black bg-opacity-60 hidden flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-md border border-slate-500 shadow-2xl">
        ...
        <div class="p-6 space-y-4">
            <div>
                <h2 id="mTaskTitle" class="text-xl font-bold text-slate-800 mb-1"></h2>
            ...
            <div>
                <span class="block text-[10px] font-bold text-slate-400 uppercase">Specifieke Dagen</span>
                <span id="mTaskDays" class="text-sm font-medium text-blue-700"></span>
            </div>
        </div>
...
<div id="editReportModal" class="fixed inset-0 bg-black bg-opacity-60 hidden flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-lg border border-slate-500 shadow-2xl">
        ...
        <form method="POST" class="p-6">
            ...
            <div class="mb-4">
                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Stemming</label>
                <select name="mood" id="e_mood" class="w-full p-2 border border-gray-300 text-sm"><option>Blij</option><option>Rustig</option><option>Pijn</option></select>
            </div>
            <div class="mb-4">
                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Inhoud</label>
...
<script>
    function switchTab(id, btn) {
        ...
        // Auto scroll chat als tab opent
    ...
    function openEditModal(report) {
        ...
        document.getElementById('e_mood').value = report.mood;
    }
